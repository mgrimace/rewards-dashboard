...
const APP_LINE_RE = /^\[([^\]]+)\]\s\[([^\]]+)\]\s\[([^\]]+)\]\s(\S+)\s\[([^\]]+)\]\s([\s\S]*)$/

const ACCOUNT_START_RE = /^Starting account:\s(\S+)\s\|\sgeoLocale:\s(.*)$/
const ACCOUNT_END_RE = /^Completed account:\s(\S+)\s\|\sTotal:\s\+(-?\d+)\s\|\sOld:\s(\d+)\s→\sNew:\s(\d+)\s\|\sDuration:\s([\d.]+)s$/
const ACCOUNT_ERR_RE = /^(\S+@\S+):\s([\s\S]*)$/
const RUN_START_RE = /^Starting Microsoft Rewards Script\s\|\sv([\w.\-]+)\s\|\sAccounts:\s(\d+)\s\|\sClusters:\s(\d+)$/
const RUN_END_RE = /^Completed all accounts\s\|\sAccounts processed:\s(\d+)\s\|\sTotal points collected:\s\+(-?\d+)\s\|\sOld total:\s(\d+)\s→\sNew total:\s(\d+)\s\|\sTotal runtime:\s([\d.]+)min$/
...
const LOGIN_NUMBER_RESOLVED_RE = /^(Approval detected|Login approved successfully|Login approval failed or timed out|Approval timeout after \d+ seconds!)$/
...
const WRAPPER_LOCK_ACQUIRED_RE = /^Lock acquired successfully\s\(PID:\s*(\d+)\)$/
...
function parseLine(rawLine) {
    ...
    switch (title) {
        case 'ACCOUNT-START': {
            ...
            if (am) return { ...base, kind: 'account-start', email: am[1], geoLocale: am[2] }
            break
        }
        case 'ACCOUNT-END': {
        ...
        case 'ACCOUNT-ERROR': {
            ...
            if (am) return { ...base, kind: 'account-error', email: am[1], error: am[2] }
        ...
        case 'LOGIN-PASSWORDLESS': {
            ...
            if (numMatch) return { ...base, kind: 'login-number', number: numMatch[1] }
...
function parseWrapperLine(dockerTs, clean) {
    ...
    if (acquired) return { ...base, kind: 'wrapper-lock-acquired', pid: acquired[1] }

    const released = WRAPPER_LOCK_RELEASED_RE.exec(message)
    if (released) return { ...base, kind: 'wrapper-lock-released', pid: released[1] }
